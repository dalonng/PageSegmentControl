...
struct ScrollableNavBarView<SelectionType>: View where SelectionType: Hashable & Sendable {
      ...
      ScrollViewReader { proxy in
        ScrollView(.horizontal, showsIndicators: false) {
          ScrollableNavBarViewLayout(spacing: internalStyle.tabItemSpacing) {
            ForEach(self.pagerSettings.itemsOrderedByIndex, id: \.self) { tag in
              NavBarItem(id: tag, selection: self.$selection)
                .tag(tag)
            }
            internalStyle.indicatorView()
              .frame(height: internalStyle.indicatorViewHeight)
              .layoutValue(key: PagerWidth.self, value: self.pagerSettings.width)
              .layoutValue(key: PagerOffset.self, value: self.pagerSettings.contentOffset)
              .animation(self.appeared ? .default : .none, value: self.pagerSettings.contentOffset)
          }
          .frame(height: internalStyle.tabItemHeight)
        }
        .background(internalStyle.barBackgroundView())
        .padding(internalStyle.padding)
        .onChange(of: self.pagerSettings.itemsOrderedByIndex) {
          if self.pagerSettings.items[self.selection] != nil {
            proxy.scrollTo(self.selection, anchor: .center)
          }
        }
        .onChange(of: self.selection) { _, newSelection in
          withAnimation {
            if self.pagerSettings.items[newSelection] != nil {
